package com.tyrengard.aureycore.foundation.common.random;
...
public final class RandomSelector<T> {
    ...
    public static <T> RandomSelector<T> weighted(
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (weight <= 0d)
                throw new IllegalArgumentException("weighter returned zero or a negative number");
    ...
    /**
     * Returns the next element using a new {@code Random} instance.
     *
     * @return
     */
    public T next() {
        return elements[selection.applyAsInt(new Random())];
    }
