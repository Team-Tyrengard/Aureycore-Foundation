...
import com.tyrengard.aureycore.foundation.common.utils.BlockUtils;
...
public class ArmorListener implements Listener {
	...
	public final void inventoryClick(final InventoryClickEvent e){
		...
		if(shift){
			...
			if(newArmorType != null){
				...
				if(newArmorType.equals(ArmorType.HELMET) && (equipping ? BlockUtils.isAirOrNull(e.getWhoClicked().getInventory().getHelmet()) : !BlockUtils.isAirOrNull(e.getWhoClicked().getInventory().getHelmet())) || newArmorType.equals(ArmorType.CHESTPLATE) && (equipping ? BlockUtils.isAirOrNull(e.getWhoClicked().getInventory().getChestplate()) : !BlockUtils.isAirOrNull(e.getWhoClicked().getInventory().getChestplate())) || newArmorType.equals(ArmorType.LEGGINGS) && (equipping ? BlockUtils.isAirOrNull(e.getWhoClicked().getInventory().getLeggings()) : !BlockUtils.isAirOrNull(e.getWhoClicked().getInventory().getLeggings())) || newArmorType.equals(ArmorType.BOOTS) && (equipping ? BlockUtils.isAirOrNull(e.getWhoClicked().getInventory().getBoots()) : !BlockUtils.isAirOrNull(e.getWhoClicked().getInventory().getBoots()))){
			...
			if(numberkey){
				if(e.getClickedInventory().getType().equals(InventoryType.PLAYER)){// Prevents shit in the 2by2 crafting
					...
					if(!BlockUtils.isAirOrNull(hotbarItem)){// Equipping
						newArmorType = ArmorType.matchType(hotbarItem);
						newArmorPiece = hotbarItem;
						oldArmorPiece = e.getClickedInventory().getItem(e.getSlot());
					}else{// Unequipping
						newArmorType = ArmorType.matchType(!BlockUtils.isAirOrNull(e.getCurrentItem()) ? e.getCurrentItem() : e.getCursor());
					}
				}
			}else{
				if(BlockUtils.isAirOrNull(e.getCursor()) && !BlockUtils.isAirOrNull(e.getCurrentItem())){// unequip with no new item going into the slot.
					newArmorType = ArmorType.matchType(e.getCurrentItem());
				}
	...
	public void playerInteractEvent(PlayerInteractEvent e){
		...
		if(e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK){
			...
			if(newArmorType != null){
				if(newArmorType.equals(ArmorType.HELMET) && BlockUtils.isAirOrNull(e.getPlayer().getInventory().getHelmet()) || newArmorType.equals(ArmorType.CHESTPLATE) && BlockUtils.isAirOrNull(e.getPlayer().getInventory().getChestplate()) || newArmorType.equals(ArmorType.LEGGINGS) && BlockUtils.isAirOrNull(e.getPlayer().getInventory().getLeggings()) || newArmorType.equals(ArmorType.BOOTS) && BlockUtils.isAirOrNull(e.getPlayer().getInventory().getBoots())){
